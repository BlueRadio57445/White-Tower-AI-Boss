...
def run_bss(config_path: str) -> dict:
    ...
    while removable:
        step += 1
        print(f"\n--- Step {step} ---")
        print(f"Current groups : {current_groups}  (score={current_score:.3f})")

        best_candidate = None
        best_score = current_score  # must not degrade (or use a threshold)
        ...
        if best_candidate is None:
            print(f"\nNo candidate can be removed without degrading score ({current_score:.3f}). Stopping.")
            log.write("no_improvement", step=step, baseline_score=round(current_score, 4))
            break

        current_groups.remove(best_candidate)
        removable.remove(best_candidate)
        extractor_tmp = SelectiveFeatureExtractor(current_groups)
        steps.append({
            "step": step,
            "removed": best_candidate,

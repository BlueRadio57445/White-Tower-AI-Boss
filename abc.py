...
def linear_prob(logits):
    """
    硬體友善版本：只用加減乘除的機率計算
    替代 Softmax：使用 ReLU + Normalize
    """
    scores = np.maximum(logits, 0) + 1e-5  # ReLU + epsilon
    total_score = np.sum(scores)
    probs = scores / total_score
    return probs, scores, total_score
...
class HybridLinearPPO:
    def __init__(self, n_feat, n_discrete_acts, gamma=0.99, lmbda=0.95, eps=0.2, hardware_friendly=False):
        """
        混合動作空間 PPO:
        - 離散動作: 前進/左轉/右轉/施法 (4個動作)
        - 連續動作: 當選擇「施法」時控制瞄準偏移
        - hardware_friendly: 若為 True，使用線性機率代替 softmax（只用加減乘除）
        """
        self.hardware_friendly = hardware_friendly
    ...
    def get_action(self, obs):
        ...
        return a_discrete, a_continuous, probs_discrete[a_discrete], mu, v, sum_scores
    ...
    def update(self, lr_a_discrete=0.003, lr_a_continuous=0.002, lr_c=0.007):
        ...
        # Buffer 格式: (obs, a_d, a_c, prob_d, mu, v, r)
        ...
        for i in range(len(self.buffer)):
            ...
            if self.hardware_friendly:
                # ===== 硬體友善模式：線性機率梯度 =====
                new_probs_discrete, new_scores, new_sum = linear_prob(logits_new)
                ...
                if (ratio_discrete * adv <= clipped_ratio_d * adv) or (1-self.eps < ratio_discrete < 1+self.eps):
                    # 線性機率的梯度公式（只用加減乘除）
                    inv_sum = 1.0 / (new_sum + 1e-8)
                    inv_prob = np.clip(1.0 / (new_probs_discrete[a_d] + 1e-8), 0, 50)
                    
                    # 更新選中的動作
                    grad_selected = adv * inv_sum * (inv_prob - 1) * s
                    grad_selected = np.clip(grad_selected, -5.0, 5.0)
                    self.w_actor_discrete[a_d] += lr_a_discrete * grad_selected
                    
                    # 更新其他動作
                    for i in range(self.n_discrete_acts):
                        if i != a_d:
                            grad_other = adv * inv_sum * (-1) * s
                            grad_other = np.clip(grad_other, -5.0, 5.0)
                            self.w_actor_discrete[i] += lr_a_discrete * grad_other
            else:
                # ===== 標準模式：Softmax 梯度 =====
                new_probs_discrete = stable_softmax(logits_new)
                ratio_discrete = new_probs_discrete[a_d] / (op_d + 1e-8)
...
# ✨ 硬體友善模式開關
# hardware_friendly=True: 只用加減乘除（ReLU + Normalize）
# hardware_friendly=False: 使用標準 Softmax
USE_HARDWARE_FRIENDLY = True  # 改為 True 可啟用硬體友善模式

agent = HybridLinearPPO(n_feat=15, n_discrete_acts=4, hardware_friendly=USE_HARDWARE_FRIENDLY)
history = []
...
print(f"硬體友善模式: {'啟用 (只用加減乘除)' if USE_HARDWARE_FRIENDLY else '停用 (使用 Softmax)'}")
epochs = 12000
...
for ep in tqdm(range(epochs)):
    ...
    total_r = 0
    for _ in range(50):
        a_d, a_c, prob_d, mu, v, sum_s = agent.get_action(obs)
        next_obs, r, event = env.step(a_d, a_c)
        agent.store((obs, a_d, a_c, prob_d, mu, v, r))  # 不存 sum_s，在 update 裡重新計算
        obs = next_obs
        total_r += r

...
| 1 | TURN_LEFT | 左轉 |
| 2 | TURN_RIGHT | 右轉 |
| 3 | CAST_SKILL | 施放技能（basic_attack）|
...
class HybridPPOAgent:
    ...
    def get_action(self, observation: np.ndarray) -> Tuple:
        ...
        action_discrete = np.random.choice(4, p=probs)
...
class Player:
    def execute_action(
        ...
        Args:
            action_discrete: 0=前進, 1=左轉, 2=右轉, 3=施法
        ...
        elif action_discrete == 1:  # TURN_LEFT
            physics.rotate_entity(self.entity, self.config.turn_speed)
            return ""

        elif action_discrete == 2:  # TURN_RIGHT
            physics.rotate_entity(self.entity, -self.config.turn_speed)
            return ""

        elif action_discrete == 3:  # CAST_SKILL
            if self.entity.skills.is_ready:
                skill_config = self.get_skill_config("basic_attack")
                skill_executor.start_cast(self.entity, skill_config, aim_offset=action_continuous)
                return "CASTING..."
...
def get_weights(self) -> dict:
    return {
        'w_actor_discrete': self.w_actor_discrete.tolist(),      # (4, 27)
        'w_actor_continuous_mu': self.w_actor_continuous_mu.tolist(),  # (27,)

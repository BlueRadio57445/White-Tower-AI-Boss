"""
Backward Stepwise Selection (BSS) for feature groups.

Greedy algorithm: starting from all features (fixed + candidates), iteratively
removes the candidate group whose removal least harms (or most improves) agent
performance. Stops when removing any candidate would degrade the current score.

Usage:
    python tools/bss.py --config tools/bss_missile.json
...
Config JSON format:
    {
        ...
        "eval_window":      50,
        "tolerance_count":  1,
        "output":           "bss_results.json"
    }

Fields:
    fixed_groups     Groups always included (never removed). Usually ["bias"].
    candidate_groups Groups to evaluate for removal. BSS removes from these greedily.
    ...
    tolerance_count  Steps allowed without matching the all-time best score.
                     In each such step the best removal is still applied.
                     Does not reset on improvement (default: 0).
    output           Path to save results JSON (default: bss_results.json).
"""
...
class _BSSLog:
    """Append-only JSONL writer for BSS progress."""

    def __init__(self, path: str):
        self._f = open(path, 'w', encoding='utf-8')
        print(f"BSS log: {path}")
...
def run_bss(config_path: str) -> dict:
    """
    Run Backward Stepwise Selection.

    Returns dict with keys:
        selected_groups  Final list of remaining group names.
        steps            List of {step, removed, score, n_features} dicts.
        final_score      Score after all removals.
    """
    ...
    # Start with all features (fixed + candidates)
    current_groups = fixed + [c for c in candidates if c not in fixed]
    removable = [c for c in current_groups if c not in fixed]

    # Derive log path from output path (replace .json → .jsonl)
    output_path = cfg.get("output", "bss_results.json")
    if not os.path.isabs(output_path):
        output_path = os.path.join(_PROJECT_DIR, output_path)
    log_path = str(Path(output_path).with_suffix(".jsonl"))
    log = _BSSLog(log_path)
    ...
    print(f"\nFixed groups   : {fixed}")
    print(f"Initial groups : {current_groups}")
    ...
    # Initial baseline score (with all features)
    print("Computing baseline score (all features)...")
    t0 = time.time()
    current_score = _evaluate(current_groups, curriculum, cfg)
    elapsed = time.time() - t0
    print(f"Baseline  groups={current_groups}  score={current_score:.3f}  ({elapsed:.1f}s)")
    log.write("baseline", groups=current_groups, score=round(current_score, 4),
              n_features=SelectiveFeatureExtractor(current_groups).n_features,
    ...
    while removable:
        step += 1
        print(f"\n--- Step {step} ---")
        print(f"Current groups : {current_groups}  (score={current_score:.3f}, best={best_ever_score:.3f}, "
        ...
        for candidate in removable:
            ...
            trial = [g for g in current_groups if g != candidate]
            t0 = time.time()
        ...
        # BSS: "genuine improvement" = at least as good as best_ever (>= not >)
        ...
        if improved:
            ...
            print(f"\nNo improvement over best ({best_ever_score:.3f}). "
                  f"Tolerance {used}/{tolerance_count}: removing '{step_best_candidate}' "
            ...
            print(f"\nNo candidate can be removed without degrading best score "
                  f"({best_ever_score:.3f}) and tolerance exhausted. Stopping.")
            log.write("no_improvement", step=step, best_ever_score=round(best_ever_score, 4))
            break

        current_groups.remove(step_best_candidate)
        removable.remove(step_best_candidate)
        extractor_tmp = SelectiveFeatureExtractor(current_groups)
        steps.append({
            "step": step,
            "removed": step_best_candidate,
            "score": round(step_best_score, 4),
            "n_features": extractor_tmp.n_features,
            "remaining": list(current_groups),
    ...
    # Summary
    final_extractor = SelectiveFeatureExtractor(current_groups)
    print("\n" + "=" * 60)
    print("BSS Complete")
    print("=" * 60)
    print(f"Remaining groups : {current_groups}")
    ...
    print(f"Visualize: python tools/visualize_bss.py --log {log_path}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Backward Stepwise Selection for feature groups"
    )
    parser.add_argument(
        "--config", required=True,
        help="Path to BSS config JSON (e.g. tools/bss_missile.json)"
    )

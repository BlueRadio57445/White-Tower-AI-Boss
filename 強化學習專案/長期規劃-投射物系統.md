...
| 檔案 | 變更 |
|-----|------|
| `game/projectile.py` | **新增** - Projectile dataclass + ProjectileManager (~150 行) |
| `core/events.py` | +5 行 - PROJECTILE_SPAWNED/HIT/DESPAWNED, DAMAGE_TAKEN, AGENT_DIED |
| `game/world.py` | +40 行 - 整合 ProjectileManager，處理遠程攻擊 |
| `rendering/pygame_renderer.py` | +80 行 - 投射物視覺化（箭矢/魔法彈） |

### 與原設計的差異

- **合併檔案**：`Projectile` 和 `ProjectileManager` 合併在 `game/projectile.py`
- **ProjectileType**：使用 Enum 而非字串常數
- **方向表示**：使用 `np.ndarray` 方向向量而非弧度角度
- **整合方式**：透過 `GameWorld._process_monster_attack()` 自動判斷 `weapon_type` 發射投射物
